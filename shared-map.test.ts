...
describe('SharedMap', () => {
  ...
  test('overwrite key', () => {
    const h = new SharedMap("string").set('key', 'old').set('key', 'new');
  ...
  test('missing key returns undefined', () => {
    const h = new SharedMap("string").set('exists', 'yes');
    expect(h.get('missing')).toBeUndefined();
  });

  test('empty string key and value', () => {
    const h = new SharedMap("string").set('', 'empty').set('key', '');
    expect(h.get('')).toBe('empty');
    expect(h.get('key')).toBe('');
  });

  test('unicode keys and values', () => {
    const h = new SharedMap("string").set('日本語', '🎉').set('émoji', '中文');
  ...
  test('long keys', () => {
    ...
    const h = new SharedMap("string").set(longKey, 'long');
  ...
  test('structural sharing - nodes reused', () => {
    const h1 = new SharedMap("string").set('a', '1').set('b', '2').set('c', '3');
    const h2 = h1.set('d', '4');
