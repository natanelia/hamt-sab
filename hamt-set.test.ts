...
describe('HAMTSet', () => {
  ...
  test('values iteration', () => {
    const s = new HAMTSet().add('a').add('b').add('c');
    const vals = [...s.values()].sort();
    expect(vals).toEqual(['a', 'b', 'c']);
  });

  test('forEach', () => {
    const s = new HAMTSet().add('x').add('y');
    const seen: string[] = [];
  ...
  test('addMany', () => {
    const s = new HAMTSet().addMany(['a', 'b', 'c', 'a']);
  ...
  test('addMany with existing returns same if no new', () => {
    const s1 = new HAMTSet().add('a').add('b');
    const s2 = s1.addMany(['a', 'b']);
    expect(s1).toBe(s2);
  });

  test('numeric values', () => {
    const s = new HAMTSet<number>().add(1).add(2).add(3);
